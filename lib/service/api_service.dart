...
abstract class APIService {
  ...
  Future<Map<String, dynamic>?> get(String url,
    ...
    try {
      ...
      UtilMethods.kPrintMessage(
          "data: ${data.body}"); // Log the response body for debugging
  ...
  String _getUrl(String url) {
    UtilMethods.kPrintMessage(
        " Url :  $BASE_URL$url"); // Log the full URL for debugging
    return BASE_URL +
        url; // Return the complete URL by concatenating the base URL with the endpoint
  }
  ...
  String _getUrlWithParams(String url,
    ...
    if (params != null) {
      ...
      UtilMethods.kPrintMessage(
          '$absUrl?${paramsString.substring(1)}'); // Log the URL with query parameters for debugging
  ...
  Map<String, dynamic>? _getResponse(http.Response response) {
    ...
    UtilMethods.kPrintMessage(
        "getting response: ,${response.body} "); // Log the raw response body
    final Map<String, dynamic> body =
        json.decode(response.body); // Decode the JSON response body
    UtilMethods.kPrintMessage(
        "parsing response : ,${response.body} "); // Log the parsed response for debugging

    // If the response status is not successful, throw an error
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw AppError(body['data'],
          null); // Custom error handling (AppError is presumably defined elsewhere)
    }
